...
import {
  ...
  studentProfileSet, studentProfileGet, myProfile,
...
function UnitsManageScreen() {
  const BLANK = { id: null, code: "", name: "", credits: 3, semester: "", deadline: "", capacity: "", fee: "" };
  const [units, setUnits] = useState(null);
...
function CollegeStudentsList() {
  const BLANK = { regNo: "", name: "", dob: "", sex: "", nationalId: "", county: "", program: "", admissionYear: "", kcseGrade: "" };
  const [rows, setRows] = useState(null);
  const [err, setErr] = useState("");
  const [msg, setMsg] = useState("");
  const [f, setF] = useState(BLANK);
  ...
  const register = async () => {
    if (!f.regNo.trim() || !f.name.trim()) return setErr("A registration number and a full name are both needed.");
    setBusy(true); setErr(""); setMsg("");
    try {
      await staffUpsert(f.regNo.trim(), f.name.trim(), "student", "student123", null);
      // The account exists the moment the name and reg number are given —
      // everything else is saved right after, but never blocks the login
      // itself from being created first.
      await studentProfileSet(f.regNo.trim(), f);
      setMsg(`${f.name.trim()} registered. Username: ${f.regNo.trim()} · Password: student123 — they choose their own the first time they sign in.`);
      setF(BLANK);
      load();
    } catch (e) { setErr(String(e.message || e).slice(0, 200)); }
    setBusy(false);
  ...
  return (
    <div>
      ...
      <div style={{ background: "#F4F8F5", border: "1px solid #DCE6E0", borderRadius: 6, padding: 14, marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
          <input value={f.regNo} onChange={(e) => setF({ ...f, regNo: e.target.value })}
            placeholder="Registration number" autoCapitalize="none"
            style={{ ...darkInput(), flex: 1, minWidth: 150, fontFamily: FONT.mono }} />
          <input value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })}
            placeholder="Full name" style={{ ...darkInput(), flex: 1, minWidth: 150 }} />
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
