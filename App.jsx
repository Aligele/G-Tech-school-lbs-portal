...
function CollegeStudentsList() {
  ...
  const [err, setErr] = useState("");
  useEffect(() => {
    staffList().then((r) => setRows((r || []).filter((s) => s.role === "student")))
      .catch((e) => { setErr(String(e.message || e).slice(0, 200)); setRows([]); });
  }, []);
  ...
  return (
    <div>
      <SectionTitle>Students</SectionTitle>
      <div style={{ fontFamily: FONT.body, fontSize: 12.5, color: "#4A5A50", marginBottom: 14 }}>
        A new student account is created from Staff logins, with the role set to Student — the
        same screen used for every other kind of account.
      </div>
      {err && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#FDE8E6", border: "1px solid #F3C0BB", fontFamily: FONT.body, fontSize: 12.5, color: "#C0261B", marginBottom: 12 }}>{err}</div>}
