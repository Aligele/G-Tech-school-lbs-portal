...
import {
  ...
  studentProfileSet, studentProfileGet, myProfile, studentRegister,
  collegeFeesList, unitFeePay, unitDelete,
...
export default function SchoolRegister() {
  ...
  return (
    <Shell>
      ...
      {role === "teacher" && !mustChange && who?.institutionType === "college" && (
        <LecturerView who={who}
  ...
  const NAV = [
    ...
    (who?.institutionType === "college"
      ? { title: "FINANCIALS", items: [
          { key: "collegefees", label: "Unit fees", icon: "fees" },
        ]}
      : { title: "FINANCIALS", items: [
    ...
    <div>
      ...
      <div style={{ maxWidth: "min(1240px, 100%)", margin: "0 auto", padding: "18px 14px 70px" }}>
        <div style={{ ...paperPanel(), padding: 22 }} className="chalk-fade paper-panel">
          ...
          {tab === "students" && who?.institutionType === "college" && <CollegeStudentsList />}

          {tab === "students" && who?.institutionType !== "college" && (
            <div>
          ...
          {tab === "units" && <UnitsManageScreen />}
          {tab === "collegett" && <CollegeTimetable />}
          {tab === "studentdocs" && <StudentDocuments />}
          {tab === "collegefees" && <CollegeFees />}
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
        <input value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })}
          placeholder={form.role === "student" ? "Registration number (used as their username)" : "Username"}
          autoCapitalize="none" style={{ ...darkInput(), flex: 1, minWidth: 130 }} />
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        <select value={form.role} onChange={(e) => {
            ...
            // A student's first password is always the same starting
            // point — theirs to change the moment they sign in, same as
            // every forced-change account already works.
            setForm({ ...form, role, password: role === "student" && !form.password ? "student123" : form.password });
          }}
          ...
          {who?.institutionType === "college" && <option value="student">Student</option>}
...
function SchoolsPanel({ who }) {
  ...
  const [f, setF] = useState({ code: "", name: "", location: "", stage: "basic", institutionType: "k12",
                               adminUser: "", adminName: "", adminPassword: "" });
  const [made, setMade] = useState(null);
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState("");

  // Resetting an admin's password in a school other than the owner's own —
  // the same duty as any admin already has for their own school, just
  // reaching further, one school at a time and only on purpose.
  const [resetFor, setResetFor] = useState(null);  // school code, or null if no row is open
  const [resetUser, setResetUser] = useState("");
  const [resetMsg, setResetMsg] = useState("");
  const [resetErr, setResetErr] = useState("");
  const [resetBusy, setResetBusy] = useState(false);

  const doReset = async (schoolCode) => {
    if (!resetUser.trim()) return setResetErr("Enter their username first.");
    setResetBusy(true); setResetErr("");
    try {
      const np = String(Math.floor(100000 + Math.random() * 900000));
      await staffResetPassword(resetUser.trim(), np, schoolCode);
      setResetMsg(`${resetUser.trim()}'s new password: ${np} — write this down, it is not shown again.`);
      setResetUser("");
    } catch (e) { setResetErr(String(e.message || e).replace(/^staff_reset_password \d+: /, "")); }
    setResetBusy(false);
  };
  ...
  const create = async () => {
    ...
    try {
      ...
      setF({ code: "", name: "", location: "", stage: "basic", institutionType: "k12",
             adminUser: "", adminName: "", adminPassword: "" });
      setAdding(false);
      await load();
  ...
  return (
    <div>
      ...
      {/* combined totals, added up from every school at once — owner only */}
      {rows && rows.length > 0 && (
        <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 100px", background: "#0A2E1A", borderRadius: 6, padding: "12px 14px" }}>
            <div style={{ fontFamily: FONT.display, fontSize: 22, fontWeight: 700, color: "#FBF7EC" }}>
              {rows.reduce((a, r) => a + (r.pupils || 0), 0)}
            </div>
            <div style={{ fontFamily: FONT.mono, fontSize: 9.5, letterSpacing: 1, color: "#B9C7BE", marginTop: 2 }}>
              PUPILS, ALL SCHOOLS
            </div>
          </div>
          <div style={{ flex: "1 1 100px", background: "#0A2E1A", borderRadius: 6, padding: "12px 14px" }}>
            <div style={{ fontFamily: FONT.display, fontSize: 22, fontWeight: 700, color: "#FBF7EC" }}>
              {rows.reduce((a, r) => a + (r.logins || 0), 0)}
            </div>
            <div style={{ fontFamily: FONT.mono, fontSize: 9.5, letterSpacing: 1, color: "#B9C7BE", marginTop: 2 }}>
              STAFF, ALL SCHOOLS
            </div>
      ...
      {adding && (
        <div className="enter" style={{ background: "#F4F8F5", border: "1px solid #DCE6E0",
          ...
          {/* Which kind of institution this is — decides everything that
              follows. A college has no fixed grade classes at all; its
              students register their own units instead. */}
          <div style={{ fontFamily: FONT.mono, fontSize: 9, letterSpacing: 1.2,
                color: "#4A5A50", textTransform: "uppercase", marginBottom: 7 }}>Kind of institution</div>
          <div style={{ display: "grid", gap: 6, marginBottom: 14, gridTemplateColumns: "1fr 1fr" }}>
            {[["k12", "School", "Primary or secondary — classes, terms, a parent's PIN"],
              ["college", "College / University", "Semesters, units a student registers themselves, a transcript"]]
              .map(([k, label, why]) => {
              const on = (f.institutionType || "k12") === k;
              return (
                <button key={k} onClick={() => setF({ ...f, institutionType: k })} className="lift"
                  style={{ textAlign: "left", padding: "10px 12px", borderRadius: 4, cursor: "pointer",
                    background: on ? "#E3F5E9" : "#FFFFFF",
                    border: `1px solid ${on ? "#0E7A3C" : "#DCE6E0"}` }}>
                  <div style={{ fontFamily: FONT.body, fontSize: 13.5, fontWeight: 700, color: "#0A2E1A" }}>{label}</div>
                  <div style={{ fontFamily: FONT.body, fontSize: 11, color: "#4A5A50", marginTop: 2 }}>{why}</div>
                </button>
              );
            })}
          </div>
          ...
          {/* Which grades the school teaches. This decides the classes it gets
              and, just as importantly, how its results are graded. Only
              relevant for an ordinary school — a college skips this
              entirely and goes straight to units. */}
          {(f.institutionType || "k12") === "k12" && (<>
          <div style={{ fontFamily: FONT.mono, fontSize: 9, letterSpacing: 1.2,
          ...
          <div key={sc.code} style={{ padding: "12px 14px", borderRadius: 5,
            ...
            {sc.active && (
              <button onClick={async () => {
                  if (!window.confirm(`Enter ${sc.name} as its admin? You'll see everything they see — pupils, marks, fees and staff.`)) return;
                  try { await ownerEnterSchool(sc.code); window.location.reload(); }
                  catch (e) { setErr(String(e.message || e)); }
                }}
                style={{ background: "none", border: "none", padding: "6px 0 0 14px", cursor: "pointer",
                  fontFamily: FONT.mono, fontSize: 11, color: "#0E7A3C", fontWeight: 700 }}>
                enter this school →
              </button>
            )}

            <button onClick={() => {
                const opening = resetFor !== sc.code;
                setResetFor(opening ? sc.code : null);
                setResetUser(""); setResetMsg(""); setResetErr("");
              }}
              style={{ background: "none", border: "none", padding: "6px 0 0 14px", cursor: "pointer",
                fontFamily: FONT.mono, fontSize: 11, color: "#0E7A3C" }}>
              {resetFor === sc.code ? "cancel" : "reset an admin's password"}
            </button>

            {resetFor === sc.code && (
              <div className="enter" style={{ marginTop: 10, padding: 12, borderRadius: 5,
                    background: "#F4F8F5", border: "1px solid #DCE6E0" }}>
                <div style={{ fontFamily: FONT.body, fontSize: 12, color: "#4A5A50",
                      marginBottom: 8, lineHeight: 1.5 }}>
                  For an admin at <strong>{sc.name}</strong> who cannot get back in — enter their
                  username exactly as they sign in with.
                </div>
                {resetMsg && (
                  <div style={{ padding: "8px 10px", borderRadius: 4, background: "#E3F5E9",
                        border: "1px solid #A9DEBC", fontFamily: FONT.mono, fontSize: 12,
                        color: "#0A2E1A", marginBottom: 8 }}>{resetMsg}</div>
                )}
                {resetErr && (
                  <div style={{ padding: "8px 10px", borderRadius: 4, background: "#FDE8E6",
                        border: "1px solid #F3C0BB", fontFamily: FONT.body, fontSize: 12,
                        color: "#C0261B", marginBottom: 8 }}>{resetErr}</div>
                )}
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <input value={resetUser} onChange={(e) => setResetUser(e.target.value)}
                    placeholder="Their username" style={{ ...darkInput(), fontFamily: FONT.mono, flex: 1 }} />
                  <button onClick={() => doReset(sc.code)} disabled={resetBusy}
                    style={{ ...primaryBtn(), fontSize: 12.5, opacity: resetBusy ? 0.5 : 1 }}>
...
// A college student's own view — registering units before their deadline,
// checking fees, and their transcript. No parent, no PIN — the student
// signs in and manages this directly, the same as any other real account.
// A lecturer's own view for a college — grading whatever is registered
// this semester, for any unit, not only ones formally assigned to them.
// Nothing here can create, edit or remove a unit — that stays an admin
// duty; a lecturer's whole job here is the roster and the grade.
function LecturerView({ who, onExit }) {
  const [units, setUnits] = useState(null);
  const [err, setErr] = useState("");
  const [roster, setRoster] = useState(null); // { unit, rows } while grading one

  const load = () => unitsManageList().then((r) => setUnits(r || []))
    .catch((e) => { setErr(String(e.message || e).slice(0, 200)); setUnits([]); });
  useEffect(() => { load(); }, []);

  const openRoster = async (u) => {
    setErr("");
    try { setRoster({ unit: u, rows: await unitRoster(u.id) || [] }); }
    catch (e) { setErr(String(e.message || e).slice(0, 200)); }
  };

  const grade = async (registrationId, g, points) => {
    try {
      await unitGrade(registrationId, g, points);
      const rows = await unitRoster(roster.unit.id);
      setRoster({ ...roster, rows: rows || [] });
    } catch (e) { setErr(String(e.message || e).slice(0, 200)); }
  };

  return (
    <div>
      <PortalHeader title={(who?.schoolName || "LECTURER").toUpperCase()}
        section={roster ? `${roster.unit.code} roster` : "Grade units"}
        onMenu={() => {}} onExit={onExit} />

      <div style={{ maxWidth: "min(900px, 100%)", margin: "0 auto", padding: "18px 14px 70px" }}>
        <div className="paper-panel" style={{ ...paperPanel(), padding: 20 }}>
          {err && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#FDE8E6", border: "1px solid #F3C0BB", fontFamily: FONT.body, fontSize: 12.5, color: "#C0261B", marginBottom: 12 }}>{err}</div>}

          {roster ? (
            <div>
              <button onClick={() => setRoster(null)} style={{ background: "none", border: "none",
                color: "#0E7A3C", fontFamily: FONT.mono, fontSize: 12, cursor: "pointer", marginBottom: 12 }}>
                ← back to units
              </button>
              <SectionTitle>{roster.unit.code} — {roster.unit.name}</SectionTitle>
              <div style={{ fontFamily: FONT.body, fontSize: 12.5, color: "#4A5A50", marginBottom: 14 }}>
                {roster.rows.length} registered this semester.
              </div>
              <div style={{ display: "grid", gap: 8 }}>
                {roster.rows.map((r) => (
                  <div key={r.registration_id} style={{ display: "flex", alignItems: "center", gap: 10,
                        padding: "10px 12px", background: "#FFFFFF", border: "1px solid #DCE6E0", borderRadius: 5, flexWrap: "wrap" }}>
                    <div style={{ flex: 1, minWidth: 140, fontFamily: FONT.body, fontSize: 13.5, color: "#0A2E1A" }}>
                      {r.student_name} <span style={{ color: "#8A9690", fontFamily: FONT.mono, fontSize: 11 }}>({r.username})</span>
                    </div>
                    <select defaultValue={r.grade || ""} onChange={(e) => {
                        const g = e.target.value;
                        const pts = { "A":4,"A-":3.7,"B+":3.3,"B":3,"B-":2.7,"C+":2.3,"C":2,"C-":1.7,"D+":1.3,"D":1,"E":0 }[g] ?? null;
                        grade(r.registration_id, g || null, pts);
                      }} style={{ ...darkInput(), width: 90, fontFamily: FONT.mono }}>
                      <option value="">—</option>
                      {["A","A-","B+","B","B-","C+","C","C-","D+","D","E"].map((g) => <option key={g} value={g}>{g}</option>)}
                    </select>
                  </div>
                ))}
              </div>
            ...
            <div>
              <SectionTitle>Grade units</SectionTitle>
              <div style={{ fontFamily: FONT.body, fontSize: 12.5, color: "#4A5A50", marginBottom: 14 }}>
                Any unit registered this semester — not only ones assigned to you.
              </div>
              {units === null ? (
                <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#4A5A50" }}>Loading…</div>
              ) : units.length === 0 ? (
                <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#5E6E64" }}>No units yet.</div>
              ) : (
                <div style={{ display: "grid", gap: 8 }}>
                  {units.map((u) => (
                    <button key={u.id} onClick={() => openRoster(u)} className="lift"
                      style={{ textAlign: "left", padding: "12px 14px", background: "#FFFFFF",
                        border: "1px solid #DCE6E0", borderRadius: 5, cursor: "pointer",
                        display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                      <div>
                        <div style={{ fontFamily: FONT.body, fontSize: 14, fontWeight: 700, color: "#0A2E1A" }}>{u.code} — {u.name}</div>
                        <div style={{ fontFamily: FONT.mono, fontSize: 11, color: "#4A5A50", marginTop: 3 }}>
                          {u.semester} · {u.registered_count} registered
                        </div>
                      </div>
                      <span style={{ color: "#0E7A3C", fontSize: 16 }}>→</span>
                    </button>
                  ))}
                </div>
              )}
...
function StudentView({ who, onExit }) {
  const [tab, setTab] = useState("profile");
  const [menuOpen, setMenuOpen] = useState(false);

  const NAV = [
    { title: "MY ACCOUNT", items: [
      { key: "profile", label: "My profile", icon: "overview" },
    ]},
    { title: "UNITS", items: [
      { key: "register", label: "Register units", icon: "students" },
      { key: "mine", label: "My units", icon: "overview" },
    ]},
    { title: "EXAMINATION", items: [
      { key: "transcript", label: "My transcript", icon: "reports" },
      { key: "examcard", label: "My exam card", icon: "reports" },
    ]},
    { title: "FEES", items: [
      { key: "fees", label: "My fees", icon: "fees" },
    ]},
  ...
  return (
    <div>
      <PortalHeader title={(who?.schoolName || "STUDENT PORTAL").toUpperCase()}
        section={NAV.flatMap((g) => g.items).find((i) => i.key === tab)?.label || tab}
        onMenu={() => setMenuOpen(true)} onExit={onExit} />
      <Sidebar open={menuOpen} onClose={() => setMenuOpen(false)} groups={NAV} active={tab} onPick={setTab}
        heading="Student" subheading={who?.name || "Student"} />

      <div style={{ maxWidth: "min(900px, 100%)", margin: "0 auto", padding: "18px 14px 70px" }}>
        <div className="paper-panel" style={{ ...paperPanel(), padding: 20 }}>
          {tab === "profile" && <MyProfileScreen who={who} onGo={setTab} />}
          {tab === "register" && <UnitRegisterScreen />}
          {tab === "mine" && <MyUnitsScreen />}
          {tab === "fees" && <MyFeesScreen who={who} />}
          {tab === "transcript" && <MyTranscriptScreen who={who} />}
          {tab === "examcard" && <MyExamCardScreen who={who} />}
          {tab === "mypassword" && <ChangeMyPassword who={who} />}
        </div>
      </div>
    </div>
...
// The very first thing a student sees — who they are, where they stand,
// and quick doors into the three things they actually came here for.
function MyProfileScreen({ who, onGo }) {
  const [rows, setRows] = useState(null);
  const [profile, setProfile] = useState(null);
  useEffect(() => { myRegistrations().then((r) => setRows(r || [])).catch(() => setRows([])); }, []);
  useEffect(() => { myProfile().then((p) => setProfile(Array.isArray(p) ? p[0] : p)).catch(() => setProfile(null)); }, []);

  const totalDue = (rows || []).reduce((a, r) => a + (Number(r.fee) || 0), 0);
  const totalPaid = (rows || []).reduce((a, r) => a + (Number(r.fee_paid) || 0), 0);
  const graded = (rows || []).filter((r) => r.grade).length;

  const row = (label, value) => value ? (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #E3DCC5" }}>
      <span style={{ fontFamily: FONT.mono, fontSize: 10.5, color: "#4A5A50" }}>{label}</span>
      <span style={{ fontFamily: FONT.body, fontSize: 13, color: "#0A2E1A" }}>{value}</span>
    </div>
  ) : null;
  ...
  return (
    <div>
      ...
      <div style={{ background: "#F4F8F5", border: "1px solid #DCE6E0", borderRadius: 8, padding: 18, marginBottom: 20 }}>
        <div style={{ fontFamily: FONT.display, fontSize: 20, fontWeight: 700, color: "#0A2E1A" }}>{who?.name}</div>
        <div style={{ fontFamily: FONT.mono, fontSize: 12, color: "#4A5A50", marginTop: 4 }}>
          REG NO. {who?.username || "—"} · {who?.schoolName}
        </div>
      </div>

      {rows !== null && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 10, marginBottom: 22 }}>
          <StatCard label="Units registered" value={rows.length} />
          <StatCard label="Units graded" value={graded} />
          <StatCard label="Fee balance" value={`KSh ${money(totalDue - totalPaid)}`}
            tone={totalDue - totalPaid > 0 ? "#C0261B" : "#0E7A3C"} />
        </div>
      )}

      {profile && (
        <div style={{ background: "#FFFFFF", border: "1px solid #DCE6E0", borderRadius: 8, padding: 16, marginBottom: 22 }}>
          {row("DEPARTMENT", profile.department)}
          {row("PROGRAM", profile.program)}
          {row("KCSE GRADE", profile.kcse_grade)}
          {row("ADMISSION YEAR", profile.admission_year)}
      ...
      <div style={{ display: "grid", gap: 8 }}>
        {[["register", "Register units", "Before this semester's deadline closes"],
          ["transcript", "My transcript", "Every unit completed, with its grade"],
          ["examcard", "My exam card", "Proof of registration, ready to print"],
          ["fees", "My fees", "What's due, what's paid, what's left"]]
          .map(([key, label, why]) => (
          <button key={key} onClick={() => onGo(key)} className="lift"
            style={{ textAlign: "left", padding: "12px 14px", borderRadius: 5, cursor: "pointer",
              background: "#FFFFFF", border: "1px solid #DCE6E0", display: "flex",
              justifyContent: "space-between", alignItems: "center" }}>
            <div>
              <div style={{ fontFamily: FONT.body, fontSize: 14, fontWeight: 700, color: "#0A2E1A" }}>{label}</div>
              <div style={{ fontFamily: FONT.body, fontSize: 11.5, color: "#4A5A50", marginTop: 2 }}>{why}</div>
            </div>
            <span style={{ color: "#0E7A3C", fontSize: 16 }}>→</span>
          </button>
...
// A student's own exam card — the same document an admin can produce for
// them, just self-service, from a student's own registrations.
function MyExamCardScreen({ who }) {
  const [rows, setRows] = useState(null);
  useEffect(() => { myRegistrations().then((r) => setRows(r || [])).catch(() => setRows([])); }, []);
  if (rows === null) return <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#4A5A50" }}>Loading…</div>;

  return (
    <div>
      <SectionTitle>My exam card</SectionTitle>
      <div className="printable" style={{ background: "#FFFFFF", border: "1px solid #DCE6E0", borderRadius: 8, padding: 26 }}>
        <div style={{ fontFamily: FONT.display, fontSize: 18, fontWeight: 700, color: "#0A2E1A" }}>Exam Card</div>
        <div style={{ fontFamily: FONT.body, fontSize: 14, color: "#0A2E1A", marginTop: 4 }}>
          {who?.name} · {who?.username}
        </div>
        {rows.length === 0 ? (
          <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#5E6E64", marginTop: 14 }}>No units registered yet.</div>
        ) : (
          <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 16 }}>
            <thead><tr style={{ borderBottom: "2px solid #0A2E1A" }}>
              {["Unit", "Semester", "Credits"].map((h) => (
                <th key={h} style={{ textAlign: "left", padding: "6px 8px", fontFamily: FONT.mono, fontSize: 10.5, color: "#4A5A50" }}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.unit_id} style={{ borderBottom: "1px solid #E3DCC5" }}>
                  <td style={{ padding: "7px 8px", fontFamily: FONT.body, fontSize: 13 }}>{r.code} — {r.name}</td>
                  <td style={{ padding: "7px 8px", fontFamily: FONT.mono, fontSize: 12 }}>{r.semester}</td>
                  <td style={{ padding: "7px 8px", fontFamily: FONT.mono, fontSize: 12 }}>{r.credits}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <button onClick={() => window.print()} style={{ ...primaryBtn(), marginTop: 14 }}>Print</button>
    </div>
  );
}

// Everything open to register for right now — greyed out the moment its
// own deadline has passed, so nobody discovers that only after trying.
function UnitRegisterScreen() {
  const [units, setUnits] = useState(null);
  const [busy, setBusy] = useState(null);
  const [msg, setMsg] = useState("");
  const [err, setErr] = useState("");
  ...
  const load = async () => {
    try { setUnits(await unitsAvailable() || []); }
    catch (e) { setErr(String(e.message || e).slice(0, 200)); setUnits([]); }
  };
  useEffect(() => { load(); }, []);
  ...
  const register = async (u) => {
    ...
    try {
      await unitRegister(u.id);
      setMsg(`Registered for ${u.code} — ${u.name}.`);
      await load();
    } catch (e) { setErr(String(e.message || e).replace(/^unit_register \d+: /, "")); }
    setBusy(null);
  };

  if (units === null) return <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#4A5A50" }}>Loading…</div>;

  return (
    <div>
      <SectionTitle>Register units</SectionTitle>
      <div style={{ fontFamily: FONT.body, fontSize: 12.5, color: "#4A5A50", marginBottom: 14 }}>
        Once a unit's own deadline has passed, it can no longer be registered here — the same
        rule applies to everyone, with no exception made after the fact.
      </div>
      ...
      {units.length === 0 && (
        <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#5E6E64" }}>
          Nothing is open for registration right now.
        </div>
      )}

      <div style={{ display: "grid", gap: 8 }}>
        {units.map((u) => {
          const closed = u.deadline_passed;
          const full = u.seats_left !== null && u.seats_left <= 0;
          const blocked = closed || full || u.already_registered;
          return (
            <div key={u.id} style={{ padding: "13px 15px", borderRadius: 5,
                  background: u.already_registered ? "#E3F5E9" : (closed || full) ? "#F4F1EA" : "#FFFFFF",
                  border: `1px solid ${u.already_registered ? "#A9DEBC" : "#DCE6E0"}`,
                  display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
              <div>
                <div style={{ fontFamily: FONT.body, fontSize: 14, fontWeight: 700, color: "#0A2E1A" }}>
                  {u.code} — {u.name}
                </div>
                <div style={{ fontFamily: FONT.mono, fontSize: 11, color: "#4A5A50", marginTop: 3 }}>
                  {u.credits} credits · {u.semester} · KSh {money(u.fee)}
                  {u.registration_deadline && (
                    <> · deadline {new Date(u.registration_deadline).toLocaleDateString()}</>
                  )}
                  {u.seats_left !== null && <> · {u.seats_left} seats left</>}
                  {u.is_common ? <> · <span style={{ color: "#B8860B" }}>common unit</span></> : (u.department && <> · {u.department}</>)}
                </div>
                {closed && <div style={{ fontFamily: FONT.mono, fontSize: 10.5, color: "#C0261B", marginTop: 3 }}>DEADLINE PASSED</div>}
                {!closed && full && <div style={{ fontFamily: FONT.mono, fontSize: 10.5, color: "#C0261B", marginTop: 3 }}>FULL</div>}
              </div>
              {u.already_registered ? (
                <div style={{ fontFamily: FONT.mono, fontSize: 11.5, color: "#0E7A3C", fontWeight: 700 }}>✓ REGISTERED</div>
              ) : (
                <button onClick={() => register(u)} disabled={blocked || busy === u.id}
                  style={{ ...primaryBtn(), fontSize: 12.5, opacity: blocked ? 0.4 : 1,
                           cursor: blocked ? "not-allowed" : "pointer" }}>
                  {busy === u.id ? "Registering…" : "Register"}
                </button>
              )}
            </div>
          );
...
// The units already registered for — and dropping one, only while its own
// deadline still allows it.
function MyUnitsScreen() {
  const [rows, setRows] = useState(null);
  const [err, setErr] = useState("");
  const [busy, setBusy] = useState(null);

  const load = async () => {
    try { setRows(await myRegistrations() || []); }
    catch (e) { setErr(String(e.message || e).slice(0, 200)); setRows([]); }
  };
  useEffect(() => { load(); }, []);

  const drop = async (unitId) => {
    if (!window.confirm("Drop this unit? This can only be undone by registering again, if the deadline allows it.")) return;
    setBusy(unitId);
    try { await unitDrop(unitId); await load(); }
    catch (e) { setErr(String(e.message || e).replace(/^unit_drop \d+: /, "")); }
    setBusy(null);
  };
  ...
  return (
    <div>
      <SectionTitle>My units</SectionTitle>
      {err && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#FDE8E6", border: "1px solid #F3C0BB", fontFamily: FONT.body, fontSize: 12.5, color: "#C0261B", marginBottom: 12 }}>{err}</div>}
      {rows.length === 0 && <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#5E6E64" }}>No units registered yet.</div>}
      <div style={{ display: "grid", gap: 8 }}>
        {rows.map((r) => (
          <div key={r.unit_id} style={{ padding: "13px 15px", borderRadius: 5, background: "#FFFFFF",
                border: "1px solid #DCE6E0", display: "flex", justifyContent: "space-between",
                alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <div>
              <div style={{ fontFamily: FONT.body, fontSize: 14, fontWeight: 700, color: "#0A2E1A" }}>{r.code} — {r.name}</div>
              <div style={{ fontFamily: FONT.mono, fontSize: 11, color: "#4A5A50", marginTop: 3 }}>
                {r.credits} credits · {r.semester}
                {r.grade && <> · grade {r.grade}</>}
              </div>
            </div>
            {!r.grade && (
              <button onClick={() => drop(r.unit_id)} disabled={busy === r.unit_id}
                style={{ background: "none", border: "1px solid #C0261B", color: "#C0261B",
                  borderRadius: 4, padding: "6px 12px", fontFamily: FONT.mono, fontSize: 11, cursor: "pointer" }}>
                {busy === r.unit_id ? "Dropping…" : "Drop"}
              </button>
            )}
          </div>
        ))}
...
function MyFeesScreen({ who }) {
  ...
  return (
    <div>
      <SectionTitle>My fees</SectionTitle>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 10, marginBottom: 20 }}>
        <StatCard label="Total due" value={`${cur} ${money(totalDue)}`} />
        <StatCard label="Paid so far" value={`${cur} ${money(totalPaid)}`} tone="#0E7A3C" />
        <StatCard label="Balance" value={`${cur} ${money(totalDue - totalPaid)}`}
          tone={totalDue - totalPaid > 0 ? "#C0261B" : "#0E7A3C"} />
      </div>
      <table style={{ width: "100%", borderCollapse: "collapse" }}>
        <thead>
          <tr style={{ borderBottom: "2px solid #0A2E1A" }}>
            {["Unit", "Semester", "Fee", "Paid"].map((h) => (
              <th key={h} style={{ textAlign: "left", padding: "6px 8px", fontFamily: FONT.mono, fontSize: 10.5, color: "#4A5A50" }}>{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {rows.map((r) => (
            <tr key={r.unit_id} style={{ borderBottom: "1px solid #E3DCC5" }}>
              <td style={{ padding: "8px", fontFamily: FONT.body, fontSize: 13 }}>{r.code} — {r.name}</td>
              <td style={{ padding: "8px", fontFamily: FONT.mono, fontSize: 12, color: "#4A5A50" }}>{r.semester}</td>
              <td style={{ padding: "8px", fontFamily: FONT.mono, fontSize: 12 }}>{cur} {money(r.fee)}</td>
              <td style={{ padding: "8px", fontFamily: FONT.mono, fontSize: 12, color: "#0E7A3C" }}>{cur} {money(r.fee_paid)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function MyTranscriptScreen({ who }) {
  const [rows, setRows] = useState(null);
  useEffect(() => { myTranscript().then((r) => setRows(r || [])).catch(() => setRows([])); }, []);
  if (rows === null) return <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#4A5A50" }}>Loading…</div>;

  const bySem = {};
  rows.forEach((r) => { (bySem[r.semester] = bySem[r.semester] || []).push(r); });

  return (
    <div>
      <SectionTitle>My transcript</SectionTitle>
      <div style={{ fontFamily: FONT.body, fontSize: 12.5, color: "#4A5A50", marginBottom: 14 }}>
        Every unit completed with a grade, exactly as recorded — nothing summarised away.
      </div>
      {rows.length === 0 && <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#5E6E64" }}>No grades recorded yet.</div>}
      {Object.entries(bySem).map(([sem, items]) => (
        <div key={sem} style={{ marginBottom: 20 }}>
          <div style={{ fontFamily: FONT.mono, fontSize: 10.5, letterSpacing: 1, color: "#0E7A3C", marginBottom: 8 }}>{sem.toUpperCase()}</div>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <tbody>
              {items.map((r) => (
                <tr key={r.code} style={{ borderBottom: "1px solid #E3DCC5" }}>
                  <td style={{ padding: "7px 8px", fontFamily: FONT.body, fontSize: 13 }}>{r.code} — {r.name}</td>
                  <td style={{ padding: "7px 8px", fontFamily: FONT.mono, fontSize: 12, color: "#4A5A50" }}>{r.credits} cr</td>
                  <td style={{ padding: "7px 8px", fontFamily: FONT.body, fontSize: 14, fontWeight: 700, color: "#0E7A3C", textAlign: "right" }}>{r.grade}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
// The admin side of units — creating them with a real deadline, and
// grading whoever registered once the semester is done.
function UnitsManageScreen() {
  const BLANK = { id: null, code: "", name: "", credits: 3, semester: "", deadline: "", capacity: "", fee: "", department: "", isCommon: false };
  const [units, setUnits] = useState(null);
  const [f, setF] = useState(BLANK);
  const [busy, setBusy] = useState(false);
  const [err, setErr] = useState("");
  const [msg, setMsg] = useState("");
  const [roster, setRoster] = useState(null);   // { unit, rows } while grading one unit

  const load = async () => {
    try { setUnits(await unitsManageList() || []); }
    catch (e) { setErr(String(e.message || e).slice(0, 200)); setUnits([]); }
  };
  useEffect(() => { load(); }, []);

  const save = async () => {
    if (!f.code.trim() || !f.name.trim() || !f.semester.trim()) {
      return setErr("A code, a name and a semester are all needed.");
    }
    setBusy(true); setErr(""); setMsg("");
    try {
      await unitUpsert({
        id: f.id, code: f.code.trim(), name: f.name.trim(), credits: Number(f.credits) || 3,
        semester: f.semester.trim(),
        deadline: f.deadline ? new Date(f.deadline).toISOString() : null,
        capacity: f.capacity ? Number(f.capacity) : null,
        fee: Number(f.fee) || 0,
        department: f.isCommon ? null : (f.department.trim() || null), isCommon: f.isCommon,
      });
      setMsg(f.id ? "Unit updated." : `${f.code.trim()} created — students can now register for it.`);
      setF(BLANK);
      await load();
    } catch (e) { setErr(String(e.message || e).replace(/^unit_upsert \d+: /, "")); }
    setBusy(false);
  };
  ...
  const removeUnit = async (u) => {
    const warning = u.registered_count > 0
      ? `${u.registered_count} student(s) are already registered for ${u.code}. Removing it removes their registration too. Continue?`
      : `Remove ${u.code} — ${u.name}?`;
    if (!window.confirm(warning)) return;
    try { await unitDelete(u.id); load(); }
    catch (e) { setErr(String(e.message || e).slice(0, 200)); }
  };

  const grade = async (registrationId, grade, points) => {
    try {
      await unitGrade(registrationId, grade, points);
      const rows = await unitRoster(roster.unit.id);
      setRoster({ ...roster, rows: rows || [] });
    } catch (e) { setErr(String(e.message || e).slice(0, 200)); }
  };
  ...
  if (roster) {
    return (
      <div>
        ...
        <SectionTitle>{roster.unit.code} — {roster.unit.name}</SectionTitle>
        <div style={{ fontFamily: FONT.body, fontSize: 12.5, color: "#4A5A50", marginBottom: 14 }}>
          {roster.rows.length} registered. A grade here is exactly what appears on that
  ...
  return (
    <div>
      ...
      <div style={{ fontFamily: FONT.body, fontSize: 12.5, color: "#4A5A50", marginBottom: 14 }}>
        What students can register for this semester. Once a deadline passes, nobody — including
        an admin working around it — can register a student after the fact from the student's own screen.
      </div>

      {msg && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#E3F5E9", border: "1px solid #A9DEBC", fontFamily: FONT.mono, fontSize: 12.5, color: "#0A2E1A", marginBottom: 12 }}>{msg}</div>}
      {err && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#FDE8E6", border: "1px solid #F3C0BB", fontFamily: FONT.body, fontSize: 12.5, color: "#C0261B", marginBottom: 12 }}>{err}</div>}

      <div style={{ background: "#F4F8F5", border: "1px solid #DCE6E0", borderRadius: 6, padding: 14, marginBottom: 20 }}>
        <div style={{ fontFamily: FONT.mono, fontSize: 10, letterSpacing: 1, color: "#4A5A50", marginBottom: 10 }}>
          {f.id ? "EDIT UNIT" : "NEW UNIT"}
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
          <input value={f.code} onChange={(e) => setF({ ...f, code: e.target.value })} placeholder="Code, e.g. CS101" style={{ ...darkInput(), flex: 1, minWidth: 110, fontFamily: FONT.mono }} />
          <input value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} placeholder="Full name" style={{ ...darkInput(), flex: 2, minWidth: 160 }} />
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
          <input value={f.semester} onChange={(e) => setF({ ...f, semester: e.target.value })} placeholder="Semester, e.g. 2026 Sem 1" style={{ ...darkInput(), flex: 1, minWidth: 140 }} />
          <input type="number" value={f.credits} onChange={(e) => setF({ ...f, credits: e.target.value })} placeholder="Credits" style={{ ...darkInput(), width: 90, fontFamily: FONT.mono }} />
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 160 }}>
            <div style={{ fontFamily: FONT.mono, fontSize: 9.5, color: "#4A5A50", marginBottom: 4 }}>REGISTRATION DEADLINE</div>
            <input type="datetime-local" value={f.deadline} onChange={(e) => setF({ ...f, deadline: e.target.value })} style={{ ...darkInput(), width: "100%", fontFamily: FONT.mono }} />
          </div>
          <input type="number" value={f.capacity} onChange={(e) => setF({ ...f, capacity: e.target.value })} placeholder="Seats (blank = unlimited)" style={{ ...darkInput(), width: 160, fontFamily: FONT.mono }} />
          <input type="number" value={f.fee} onChange={(e) => setF({ ...f, fee: e.target.value })} placeholder="Fee (KSh)" style={{ ...darkInput(), width: 120, fontFamily: FONT.mono }} />
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 12, flexWrap: "wrap" }}>
          <input value={f.department} onChange={(e) => setF({ ...f, department: e.target.value })}
            placeholder="Department, e.g. Business" disabled={f.isCommon}
            style={{ ...darkInput(), flex: 1, minWidth: 160, opacity: f.isCommon ? 0.5 : 1 }} />
          <label style={{ display: "flex", alignItems: "center", gap: 6, fontFamily: FONT.body, fontSize: 12.5, color: "#0A2E1A", cursor: "pointer" }}>
            <input type="checkbox" checked={f.isCommon} onChange={(e) => setF({ ...f, isCommon: e.target.checked, department: e.target.checked ? "" : f.department })} />
            Common unit — open to every department
          </label>
        </div>
        <button onClick={save} disabled={busy} style={{ ...primaryBtn(), opacity: busy ? 0.5 : 1 }}>
          {busy ? "Saving…" : f.id ? "Save changes" : "Create unit"}
        </button>
        {f.id && <button onClick={() => setF(BLANK)} style={{ ...backBtnStyle(), marginLeft: 10 }}>cancel</button>}
      </div>

      {units === null ? (
        <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#4A5A50" }}>Loading…</div>
      ) : units.length === 0 ? (
        <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#5E6E64" }}>No units yet — create the first one above.</div>
      ) : (
        <div style={{ display: "grid", gap: 8 }}>
          {units.map((u) => (
            <div key={u.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between",
                  gap: 10, padding: "12px 14px", background: "#FFFFFF", border: "1px solid #DCE6E0", borderRadius: 5, flexWrap: "wrap" }}>
              <div>
                <div style={{ fontFamily: FONT.body, fontSize: 14, fontWeight: 700, color: "#0A2E1A" }}>{u.code} — {u.name}</div>
                <div style={{ fontFamily: FONT.mono, fontSize: 11, color: "#4A5A50", marginTop: 3 }}>
                  {u.semester} · {u.registered_count} registered
                  {u.registration_deadline && <> · closes {new Date(u.registration_deadline).toLocaleDateString()}</>}
                </div>
                <div style={{ fontFamily: FONT.mono, fontSize: 10.5, marginTop: 4 }}>
                  {u.is_common
                    ? <span style={{ color: "#B8860B" }}>COMMON — EVERY DEPARTMENT</span>
                    : <span style={{ color: "#0E7A3C" }}>{u.department || "NO DEPARTMENT SET"}</span>}
                </div>
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => openRoster(u)} style={{ ...backBtnStyle() }}>roster & grade</button>
                <button onClick={() => setF({ id: u.id, code: u.code, name: u.name, credits: u.credits,
                    semester: u.semester, deadline: u.registration_deadline ? u.registration_deadline.slice(0,16) : "",
                    capacity: u.capacity || "", fee: u.fee, department: u.department || "", isCommon: !!u.is_common })}
                  style={{ background: "none", border: "1px solid #DCE6E0", borderRadius: 4, padding: "6px 12px",
                    fontFamily: FONT.mono, fontSize: 11, cursor: "pointer", color: "#0A2E1A" }}>edit</button>
                <button onClick={() => removeUnit(u)}
                  style={{ background: "none", border: "1px solid #F3C0BB", borderRadius: 4, padding: "6px 12px",
                    fontFamily: FONT.mono, fontSize: 11, cursor: "pointer", color: "#C0261B" }}>remove</button>
              </div>
            </div>
          ))}
        </div>
...
// Just the student accounts — a college's students live in the same staff
// table as everyone else, since they sign in directly. This is the same
// list Staff logins already has, filtered down to one role.
function CollegeStudentsList() {
  const BLANK = { name: "", dob: "", sex: "", nationalId: "", county: "", department: "", program: "", admissionYear: "", kcseGrade: "" };
  const [rows, setRows] = useState(null);
  const [err, setErr] = useState("");
  const [msg, setMsg] = useState("");
  const [f, setF] = useState(BLANK);
  const [busy, setBusy] = useState(false);
  const [viewing, setViewing] = useState(null); // full profile of one student, once opened
  const [editing, setEditing] = useState(false);
  const [editForm, setEditForm] = useState({});

  const load = () => staffList().then((r) => setRows((r || []).filter((s) => s.role === "student")))
    .catch((e) => { setErr(String(e.message || e).slice(0, 200)); setRows([]); });
  useEffect(() => { load(); }, []);

  const register = async () => {
    if (!f.name.trim()) return setErr("The student needs a name.");
    setBusy(true); setErr(""); setMsg("");
    try {
      // The system works out the registration number itself, so nobody
      // types one in and nobody can type in one that already exists.
      const rows2 = await studentRegister(f);
      const regNo = (Array.isArray(rows2) ? rows2[0] : rows2)?.reg_no;
      setMsg(`${f.name.trim()} registered. Registration number: ${regNo} · Password: student123 — they choose their own the first time they sign in.`);
      setF(BLANK);
      load();
    } catch (e) { setErr(String(e.message || e).slice(0, 200)); }
    setBusy(false);
  ...
  const openProfile = async (username) => {
    ...
    try { setViewing(await studentProfileGet(username)); }
    catch (e) { setErr(String(e.message || e).slice(0, 200)); }
  };
  ...
  if (viewing) {
    const v = Array.isArray(viewing) ? viewing[0] : viewing;
    const row = (label, value) => (
      <div style={{ display: "flex", justifyContent: "space-between", padding: "9px 0", borderBottom: "1px solid #E3DCC5" }}>
        <span style={{ fontFamily: FONT.mono, fontSize: 11, color: "#4A5A50" }}>{label}</span>
        <span style={{ fontFamily: FONT.body, fontSize: 13.5, color: "#0A2E1A" }}>{value || "—"}</span>
      </div>
    );
    ...
    if (editing) {
      const saveEdit = async () => {
        setErr(""); setMsg("");
        try {
          await studentProfileSet(v.username, editForm);
          setMsg("Saved.");
          setEditing(false);
          const fresh = await studentProfileGet(v.username);
          setViewing(fresh);
        } catch (e) { setErr(String(e.message || e).slice(0, 200)); }
      };
      return (
        <div>
          <button onClick={() => setEditing(false)} style={{ background: "none", border: "none",
            color: "#0E7A3C", fontFamily: FONT.mono, fontSize: 12, cursor: "pointer", marginBottom: 12 }}>
            ← cancel
          </button>
          <SectionTitle>Edit {v?.name}</SectionTitle>
          {err && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#FDE8E6", border: "1px solid #F3C0BB", fontFamily: FONT.body, fontSize: 12.5, color: "#C0261B", marginBottom: 12 }}>{err}</div>}
          <div style={{ background: "#FFFFFF", border: "1px solid #DCE6E0", borderRadius: 8, padding: 18 }}>
            <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
              <select value={editForm.sex} onChange={(e) => setEditForm({ ...editForm, sex: e.target.value })} style={{ ...darkInput(), width: 110 }}>
                <option value="">Sex…</option><option value="Female">Female</option><option value="Male">Male</option>
              </select>
              <input type="date" value={editForm.dob} onChange={(e) => setEditForm({ ...editForm, dob: e.target.value })} style={{ ...darkInput(), flex: 1, minWidth: 130, fontFamily: FONT.mono }} />
              <input value={editForm.nationalId} onChange={(e) => setEditForm({ ...editForm, nationalId: e.target.value })} placeholder="National ID" style={{ ...darkInput(), flex: 1, minWidth: 120, fontFamily: FONT.mono }} />
            </div>
            <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
              <input value={editForm.county} onChange={(e) => setEditForm({ ...editForm, county: e.target.value })} placeholder="County" style={{ ...darkInput(), flex: 1, minWidth: 120 }} />
              <input value={editForm.program} onChange={(e) => setEditForm({ ...editForm, program: e.target.value })} placeholder="Program / course" style={{ ...darkInput(), flex: 2, minWidth: 160 }} />
            </div>
            <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
              <input value={editForm.department} onChange={(e) => setEditForm({ ...editForm, department: e.target.value })} placeholder="Department" style={{ ...darkInput(), flex: 1, minWidth: 160 }} />
            </div>
            <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
              <input type="number" value={editForm.admissionYear} onChange={(e) => setEditForm({ ...editForm, admissionYear: e.target.value })} placeholder="Admission year" style={{ ...darkInput(), width: 140, fontFamily: FONT.mono }} />
              <input value={editForm.kcseGrade} onChange={(e) => setEditForm({ ...editForm, kcseGrade: e.target.value })} placeholder="KCSE grade" style={{ ...darkInput(), width: 140, fontFamily: FONT.mono }} />
            </div>
    ...
    return (
      <div>
        <button onClick={() => { setViewing(null); setEditing(false); }} style={{ background: "none", border: "none",
          color: "#0E7A3C", fontFamily: FONT.mono, fontSize: 12, cursor: "pointer", marginBottom: 12 }}>
          ← back to students
        </button>
        <SectionTitle>{v?.name}</SectionTitle>
        {msg && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#E3F5E9", border: "1px solid #A9DEBC", fontFamily: FONT.mono, fontSize: 12, color: "#0A2E1A", marginBottom: 12 }}>{msg}</div>}
        <div style={{ background: "#FFFFFF", border: "1px solid #DCE6E0", borderRadius: 8, padding: 18 }}>
          {row("REGISTRATION NUMBER", v?.username)}
          {row("KCSE GRADE", v?.kcse_grade)}
          {row("DEPARTMENT", v?.department)}
          {row("PROGRAM", v?.program)}
          {row("ADMISSION YEAR", v?.admission_year)}
          {row("DATE OF BIRTH", v?.dob)}
          {row("SEX", v?.sex)}
          {row("NATIONAL ID", v?.national_id)}
          {row("COUNTY", v?.county)}
          {row("EMAIL", v?.email)}
          {row("PHONE", v?.phone)}
        </div>
        <button onClick={() => {
            setEditForm({ dob: v?.dob || "", sex: v?.sex || "", nationalId: v?.national_id || "",
              county: v?.county || "", department: v?.department || "", program: v?.program || "",
              admissionYear: v?.admission_year || "", kcseGrade: v?.kcse_grade || "" });
            setEditing(true);
          }}
          style={{ ...primaryBtn(), marginTop: 14 }}>Edit this record</button>
      </div>
    );
  }
  ...
  return (
    <div>
      <SectionTitle>Register a student</SectionTitle>
      <div style={{ fontFamily: FONT.body, fontSize: 12.5, color: "#4A5A50", marginBottom: 14 }}>
        Their registration number becomes their username. Every new student starts with the same
        password — <strong>student123</strong> — and is asked to choose their own the moment they
        first sign in.
      </div>

      {msg && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#E3F5E9", border: "1px solid #A9DEBC", fontFamily: FONT.mono, fontSize: 12, color: "#0A2E1A", marginBottom: 12, lineHeight: 1.5 }}>{msg}</div>}
      {err && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#FDE8E6", border: "1px solid #F3C0BB", fontFamily: FONT.body, fontSize: 12.5, color: "#C0261B", marginBottom: 12 }}>{err}</div>}

      <div style={{ background: "#F4F8F5", border: "1px solid #DCE6E0", borderRadius: 6, padding: 14, marginBottom: 24 }}>
        <div style={{ fontFamily: FONT.mono, fontSize: 9.5, letterSpacing: 1, color: "#4A5A50", marginBottom: 8 }}>THE STUDENT</div>
        <div style={{ fontFamily: FONT.body, fontSize: 11.5, color: "#4A5A50", marginBottom: 10 }}>
          The registration number is assigned automatically once saved — nothing to type here.
        </div>
        <div style={{ marginBottom: 8 }}>
          <input value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })}
            placeholder="Full name" style={{ ...darkInput(), width: "100%" }} />
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
          <select value={f.sex} onChange={(e) => setF({ ...f, sex: e.target.value })} style={{ ...darkInput(), width: 110 }}>
            <option value="">Sex…</option><option value="Female">Female</option><option value="Male">Male</option>
          </select>
          <input type="date" value={f.dob} onChange={(e) => setF({ ...f, dob: e.target.value })}
            style={{ ...darkInput(), flex: 1, minWidth: 130, fontFamily: FONT.mono }} />
          <input value={f.nationalId} onChange={(e) => setF({ ...f, nationalId: e.target.value })}
            placeholder="National ID" style={{ ...darkInput(), flex: 1, minWidth: 120, fontFamily: FONT.mono }} />
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
          <input value={f.county} onChange={(e) => setF({ ...f, county: e.target.value })}
            placeholder="County" style={{ ...darkInput(), flex: 1, minWidth: 120 }} />
          <input value={f.program} onChange={(e) => setF({ ...f, program: e.target.value })}
            placeholder="Program / course" style={{ ...darkInput(), flex: 2, minWidth: 160 }} />
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
          <input value={f.department} onChange={(e) => setF({ ...f, department: e.target.value })}
            placeholder="Department, e.g. Business, Technology, Nursing"
            style={{ ...darkInput(), flex: 1, minWidth: 200 }} />
        </div>
        <div style={{ fontFamily: FONT.mono, fontSize: 9.5, letterSpacing: 1, color: "#4A5A50", margin: "10px 0 8px" }}>ADMISSION</div>
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          <input type="number" value={f.admissionYear} onChange={(e) => setF({ ...f, admissionYear: e.target.value })}
            placeholder="Admission year" style={{ ...darkInput(), width: 140, fontFamily: FONT.mono }} />
          <input value={f.kcseGrade} onChange={(e) => setF({ ...f, kcseGrade: e.target.value })}
            placeholder="KCSE grade, e.g. B+" style={{ ...darkInput(), width: 160, fontFamily: FONT.mono }} />
        </div>
        <button onClick={register} disabled={busy} style={{ ...primaryBtn(), opacity: busy ? 0.5 : 1 }}>
          {busy ? "Registering…" : "Register student"}
        </button>
      </div>

      <SectionTitle>Students</SectionTitle>
      {rows.length === 0 && <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#5E6E64" }}>No students registered yet.</div>}
      <div style={{ display: "grid", gap: 8 }}>
        {rows.map((s) => (
          <button key={s.id} onClick={() => openProfile(s.username)} className="lift"
            style={{ padding: "11px 14px", background: "#FFFFFF", border: "1px solid #DCE6E0", textAlign: "left", cursor: "pointer",
                borderRadius: 5, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            <div style={{ fontFamily: FONT.body, fontSize: 14, color: "#0A2E1A" }}>{s.name}</div>
            <div style={{ fontFamily: FONT.mono, fontSize: 11.5, color: "#4A5A50" }}>
              {s.username} {!s.active && "· disabled"}
            </div>
          </button>
        ))}
      </div>
...
// One shared timetable per semester — a college has no fixed classes to
// key this by, so it is kept as one flexible grid instead.
function CollegeTimetable() {
  ...
  const load = async (sem) => {
    if (!sem.trim()) return;
    try { setGrid((await collegeTimetableGet(sem)) || {}); } catch (e) { setGrid({}); }
  };

  const save = async () => {
    if (!semester.trim()) return;
    setBusy(true); setMsg("");
    try { await collegeTimetableSet(semester, grid); setMsg("Saved."); }
    catch (e) { setMsg(String(e.message || e).slice(0, 160)); }
    setBusy(false);
  };

  const setCell = (day, text) => setGrid({ ...grid, [day]: text });

  return (
    <div>
      <SectionTitle>Timetable</SectionTitle>
      <div style={{ fontFamily: FONT.body, fontSize: 12.5, color: "#4A5A50", marginBottom: 14 }}>
        One timetable for the whole semester — free text, since a college's own schedule rarely
        fits a fixed grid the way a single class's does.
      </div>
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <input value={semester} onChange={(e) => setSemester(e.target.value)} onBlur={() => load(semester)}
          placeholder="Semester, e.g. 2026 Sem 1" style={{ ...darkInput(), flex: 1 }} />
      </div>
      {semester.trim() && (
        <>
          <div style={{ display: "grid", gap: 10 }}>
            {DAYS.map((d) => (
              <div key={d}>
                <div style={{ fontFamily: FONT.mono, fontSize: 10.5, color: "#4A5A50", marginBottom: 4 }}>{d.toUpperCase()}</div>
                <textarea value={grid[d] || ""} onChange={(e) => setCell(d, e.target.value)}
                  placeholder="e.g. 9:00 CS101 Room 4 — 11:00 CS102 Room 2"
                  style={{ ...darkInput(), width: "100%", height: 56, resize: "vertical" }} />
              </div>
            ))}
          </div>
          <button onClick={save} disabled={busy} style={{ ...primaryBtn(), marginTop: 14, opacity: busy ? 0.5 : 1 }}>
            {busy ? "Saving…" : "Save timetable"}
          </button>
          {msg && <span style={{ marginLeft: 12, fontFamily: FONT.mono, fontSize: 12, color: "#0E7A3C" }}>{msg}</span>}
        </>
      )}
    </div>
  );
...
// Two documents a college hands out constantly: what a student owes, and
// proof of what they are registered for, needed to actually sit an exam.
function StudentDocuments() {
  const [username, setUsername] = useState("");
  const [rows, setRows] = useState(null);
  const [err, setErr] = useState("");
  const [showing, setShowing] = useState(null); // "statement" | "examcard" | null

  const search = async () => {
    if (!username.trim()) return;
    setErr(""); setRows(null); setShowing(null);
    try { setRows(await studentRegistrations(username.trim()) || []); }
    catch (e) { setErr(String(e.message || e).replace(/^student_registrations \d+: /, "")); }
  };

  if (showing && rows) {
    const name = rows[0]?.student_name || username;
    const total = rows.reduce((a, r) => a + Number(r.fee || 0), 0);
    const paid = rows.reduce((a, r) => a + Number(r.fee_paid || 0), 0);
    return (
      <div>
        <button onClick={() => setShowing(null)} style={{ background: "none", border: "none",
          color: "#0E7A3C", fontFamily: FONT.mono, fontSize: 12, cursor: "pointer", marginBottom: 12 }}>
          ← back
        </button>
        <div className="printable" style={{ background: "#FFFFFF", border: "1px solid #DCE6E0", borderRadius: 8, padding: 26 }}>
          <div style={{ fontFamily: FONT.display, fontSize: 18, fontWeight: 700, color: "#0A2E1A" }}>
            {showing === "statement" ? "Fee Statement" : "Exam Card"}
          </div>
          <div style={{ fontFamily: FONT.body, fontSize: 14, color: "#0A2E1A", marginTop: 4 }}>{name} · {username}</div>
          <table style={{ width: "100%", borderCollapse: "collapse", marginTop: 16 }}>
            <thead><tr style={{ borderBottom: "2px solid #0A2E1A" }}>
              {(showing === "statement" ? ["Unit","Semester","Fee","Paid"] : ["Unit","Semester","Credits"]).map((h) => (
                <th key={h} style={{ textAlign: "left", padding: "6px 8px", fontFamily: FONT.mono, fontSize: 10.5, color: "#4A5A50" }}>{h}</th>
              ))}
            </tr></thead>
            <tbody>
              {rows.map((r) => (
                <tr key={r.unit_id} style={{ borderBottom: "1px solid #E3DCC5" }}>
                  ...
                  <td style={{ padding: "7px 8px", fontFamily: FONT.mono, fontSize: 12 }}>{r.semester}</td>
                  {showing === "statement" ? (<>
                    <td style={{ padding: "7px 8px", fontFamily: FONT.mono, fontSize: 12 }}>KSh {money(r.fee)}</td>
                    <td style={{ padding: "7px 8px", fontFamily: FONT.mono, fontSize: 12, color: "#0E7A3C" }}>KSh {money(r.fee_paid)}</td>
                  </>) : (
                    <td style={{ padding: "7px 8px", fontFamily: FONT.mono, fontSize: 12 }}>{r.credits}</td>
                  )}
                </tr>
          ...
          {showing === "statement" && (
            <div style={{ marginTop: 16, paddingTop: 12, borderTop: "2px solid #0A2E1A",
                  display: "flex", justifyContent: "space-between", fontFamily: FONT.mono, fontSize: 13 }}>
              <span>Balance</span><span style={{ fontWeight: 700 }}>KSh {money(total - paid)}</span>
            </div>
          )}
        </div>
        <button onClick={() => window.print()} style={{ ...primaryBtn(), marginTop: 14 }}>Print</button>
  ...
  return (
    <div>
      <SectionTitle>Fee statement / exam card</SectionTitle>
      <div style={{ fontFamily: FONT.body, fontSize: 12.5, color: "#4A5A50", marginBottom: 14 }}>
        Find a student by their username to produce either document from what they are actually
        registered for right now.
      </div>
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Student's username"
          style={{ ...darkInput(), flex: 1, fontFamily: FONT.mono }} />
        <button onClick={search} style={{ ...primaryBtn(), fontSize: 12.5 }}>Find</button>
      </div>
      {err && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#FDE8E6", border: "1px solid #F3C0BB", fontFamily: FONT.body, fontSize: 12.5, color: "#C0261B" }}>{err}</div>}
      {rows && rows.length === 0 && <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#5E6E64" }}>No registrations found for that student.</div>}
      {rows && rows.length > 0 && (
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => setShowing("statement")} style={{ ...primaryBtn() }}>Fee statement</button>
          <button onClick={() => setShowing("examcard")} style={{ ...primaryBtn(), background: "#0A2E1A" }}>Exam card</button>
        </div>
      )}
    </div>
  );
...
// Every unit fee across the whole college, in one place — recording a
// payment here updates the exact registration it belongs to.
function CollegeFees() {
  ...
  const load = () => collegeFeesList().then((r) => setRows(r || []))
    .catch((e) => { setErr(String(e.message || e).slice(0, 200)); setRows([]); });
  useEffect(() => { load(); }, []);

  const pay = async (registrationId) => {
    const amt = Number(amount);
    if (!amt || amt <= 0) return setErr("Enter a real amount.");
    try {
      await unitFeePay(registrationId, amt);
      setMsg(`KSh ${money(amt)} recorded.`);
  ...
  return (
    <div>
      <SectionTitle>Unit fees</SectionTitle>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(120px,1fr))", gap: 10, marginBottom: 20 }}>
        <StatCard label="Total due" value={`KSh ${money(totalDue)}`} />
        <StatCard label="Collected" value={`KSh ${money(totalPaid)}`} tone="#0E7A3C" />
        <StatCard label="Outstanding" value={`KSh ${money(totalDue - totalPaid)}`}
          tone={totalDue - totalPaid > 0 ? "#C0261B" : "#0E7A3C"} />
      </div>

      {msg && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#E3F5E9", border: "1px solid #A9DEBC", fontFamily: FONT.mono, fontSize: 12, color: "#0A2E1A", marginBottom: 12 }}>{msg}</div>}
      {err && <div style={{ padding: "9px 12px", borderRadius: 4, background: "#FDE8E6", border: "1px solid #F3C0BB", fontFamily: FONT.body, fontSize: 12.5, color: "#C0261B", marginBottom: 12 }}>{err}</div>}
      {rows.length === 0 && <div style={{ fontFamily: FONT.body, fontSize: 13, color: "#5E6E64" }}>No registrations yet.</div>}

      <div style={{ display: "grid", gap: 8 }}>
        {rows.map((r) => {
          const bal = Number(r.fee) - Number(r.fee_paid);
          return (
            <div key={r.registration_id} style={{ padding: "12px 14px", background: "#FFFFFF",
                  border: "1px solid #DCE6E0", borderRadius: 5 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                <div>
                  <div style={{ fontFamily: FONT.body, fontSize: 14, fontWeight: 700, color: "#0A2E1A" }}>{r.student_name}</div>
                  <div style={{ fontFamily: FONT.mono, fontSize: 11, color: "#4A5A50", marginTop: 2 }}>
                    {r.username} · {r.code} — {r.name} · {r.semester}
                  </div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontFamily: FONT.mono, fontSize: 12.5, color: bal > 0 ? "#C0261B" : "#0E7A3C" }}>
                    KSh {money(r.fee_paid)} / {money(r.fee)}
                  </div>
                  {bal > 0 && (
                    <button onClick={() => { setPaying(r.registration_id); setAmount(""); setErr(""); }}
                      style={{ ...backBtnStyle(), marginTop: 4 }}>record payment</button>
                  )}
                </div>
              </div>
              {paying === r.registration_id && (
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  <input type="number" value={amount} onChange={(e) => setAmount(e.target.value)}
                    placeholder="Amount (KSh)" style={{ ...darkInput(), flex: 1, fontFamily: FONT.mono }} />
                  <button onClick={() => pay(r.registration_id)} style={{ ...primaryBtn(), fontSize: 12.5 }}>Save</button>
                  <button onClick={() => setPaying(null)} style={{ ...backBtnStyle() }}>cancel</button>
                </div>
              )}
            </div>
          );
