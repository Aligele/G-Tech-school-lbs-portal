...
function TeacherAddStudent({ roster, saveRoster, classId, actorName }) {
  // Same fields, same saved shape as the admin's own registration screen —
  // a pupil registered by a teacher is not a lighter, second-class record
  // that someone has to go back and complete later.
  const BLANK_STUDENT = {
    admNo: "",
  ...
  const [n, setN] = useState(BLANK_STUDENT);
  const [msg, setMsg] = useState("");
  const mine = roster.students.filter((s) => s.classId === classId);

  const add = () => {
    if (!n.firstName.trim() || !n.surname.trim()) return;
    const pin = String(Math.floor(1000 + Math.random() * 9000));
    const display = [n.firstName, n.middleName, n.surname]
      .map((x) => x.trim()).filter(Boolean).join(" ");
    const keep = (v) => { const t = String(v ?? "").trim(); return t === "" ? undefined : t; };
    ...
    const s = {
      id: typed || nextAdmissionNo(roster.students),
      name: display,
      firstName: n.firstName.trim(), middleName: keep(n.middleName), surname: n.surname.trim(),
      classId,
      sex: n.sex || undefined, dob: keep(n.dob), birthCertNo: keep(n.birthCertNo),
      nationality: keep(n.nationality) || "Kenyan", assessmentNo: keep(n.assessmentNo),
      county: keep(n.county), subCounty: keep(n.subCounty), ward: keep(n.ward), homeArea: keep(n.homeArea),
      parentName: n.parentName.trim(), parentRelation: n.parentRelation || undefined,
      parentId: keep(n.parentId), parentPhone: keep(n.parentPhone), parentEmail: keep(n.parentEmail),
      altPhone: keep(n.altPhone), medical: keep(n.medical), boarder: n.boarder || undefined,
      feeDue: Number(n.feeDue) || 0, feePaid: 0, payments: [], pin,
      admittedOn: todayISO(),
    };
    const next = logAction({ ...roster, students: [...roster.students, s] },
      actorName || "Teacher", `Registered pupil ${display} (${s.id})`);
    saveRoster(next, `${display} added — Adm ${s.id}, PIN ${pin}`);
    setMsg(`${display} registered. Admission No: ${s.id} · PIN: ${pin} — write these down for the parent.`);
    setN({ ...BLANK_STUDENT, county: n.county, subCounty: n.subCounty, ward: n.ward, feeDue: n.feeDue });
  };
  ...
  return (
    <div>
      ...
      <div style={{ fontFamily: FONT.body, fontSize: 12.5, color: "#4A5A50", marginBottom: 12 }}>
        New pupils go straight into <strong>{classNameOf(roster, classId)}</strong>.
        Only the name is required today — the rest can be filled in later rather than turning a family away.
      ...
      {msg && (
        <div style={{ padding: "10px 12px", borderRadius: 4, background: "#E3F5E9", border: "1px solid #A9DEBC",
                      fontFamily: FONT.mono, fontSize: 12.5, color: "#0A2E1A", marginBottom: 14 }}>{msg}</div>
      )}

      <div style={{ display: "grid", gap: 14, marginBottom: 16 }}>
        <FormCard title="The pupil">
          <FormRow>
            <Field label="Admission number" hint={`Leave blank and the school gives it ${nextAdmissionNo(roster.students)}`}>
              <input value={n.admNo} onChange={(e) => setN({ ...n, admNo: e.target.value })}
                placeholder={nextAdmissionNo(roster.students)} style={{ ...darkInput(), width: "100%", fontFamily: FONT.mono }} />
            </Field>
            <Field label="First name" required>
              <input value={n.firstName} onChange={(e) => setN({ ...n, firstName: e.target.value })} style={{ ...darkInput(), width: "100%" }} />
            </Field>
            <Field label="Middle name">
              <input value={n.middleName} onChange={(e) => setN({ ...n, middleName: e.target.value })} style={{ ...darkInput(), width: "100%" }} />
            </Field>
            <Field label="Surname" required>
              <input value={n.surname} onChange={(e) => setN({ ...n, surname: e.target.value })} style={{ ...darkInput(), width: "100%" }} />
            </Field>
          </FormRow>
          <FormRow>
            <Field label="Boy or girl">
              <select value={n.sex} onChange={(e) => setN({ ...n, sex: e.target.value })} style={{ ...darkInput(), width: "100%" }}>
                <option value="">Choose…</option>
                <option value="M">Boy</option>
                <option value="F">Girl</option>
              </select>
            ...
            <Field label="Date of birth">
              <input type="date" value={n.dob} onChange={(e) => setN({ ...n, dob: e.target.value })} style={{ ...darkInput(), width: "100%" }} />
            </Field>
            <Field label="Nationality">
              <input value={n.nationality} onChange={(e) => setN({ ...n, nationality: e.target.value })} style={{ ...darkInput(), width: "100%" }} />
            </Field>
          </FormRow>
          <FormRow>
            <Field label="Birth certificate number">
              <input value={n.birthCertNo} onChange={(e) => setN({ ...n, birthCertNo: e.target.value })}
                placeholder="e.g. BC/2018/44711" style={{ ...darkInput(), width: "100%", fontFamily: FONT.mono }} />
            </Field>
            <Field label="Assessment number" hint="KNEC, if the pupil has one">
              <input value={n.assessmentNo} onChange={(e) => setN({ ...n, assessmentNo: e.target.value })} style={{ ...darkInput(), width: "100%", fontFamily: FONT.mono }} />
            </Field>
          </FormRow>
        </FormCard>
        ...
        <FormCard title="Where the family lives">
          <FormRow>
            <Field label="County">
              <select value={n.county} onChange={(e) => setN({ ...n, county: e.target.value })} style={{ ...darkInput(), width: "100%" }}>
                <option value="">Choose a county…</option>
                {KENYA_COUNTIES.map((c) => <option key={c} value={c}>{c}</option>)}
              </select>
            </Field>
            <Field label="Sub-county">
              <input value={n.subCounty} onChange={(e) => setN({ ...n, subCounty: e.target.value })} placeholder="e.g. Wajir South" style={{ ...darkInput(), width: "100%" }} />
            </Field>
            <Field label="Ward">
              <input value={n.ward} onChange={(e) => setN({ ...n, ward: e.target.value })} placeholder="e.g. Benane" style={{ ...darkInput(), width: "100%" }} />
            </Field>
          </FormRow>
          <FormRow>
            <Field label="Village or estate" hint="Enough to find the home if a child is unwell">
              <input value={n.homeArea} onChange={(e) => setN({ ...n, homeArea: e.target.value })} style={{ ...darkInput(), width: "100%" }} />
            </Field>
          </FormRow>
        </FormCard>
        ...
        <FormCard title="Parent or guardian">
          <FormRow>
            <Field label="Full name">
              <input value={n.parentName} onChange={(e) => setN({ ...n, parentName: e.target.value })} style={{ ...darkInput(), width: "100%" }} />
            </Field>
            <Field label="Relationship to the pupil">
              <select value={n.parentRelation} onChange={(e) => setN({ ...n, parentRelation: e.target.value })} style={{ ...darkInput(), width: "100%" }}>
                {RELATIONSHIPS.map((r) => <option key={r} value={r}>{r}</option>)}
              </select>
            </Field>
            <Field label="National ID number">
              <input value={n.parentId} onChange={(e) => setN({ ...n, parentId: e.target.value })} style={{ ...darkInput(), width: "100%", fontFamily: FONT.mono }} />
            </Field>
          </FormRow>
          <FormRow>
            <Field label="Phone" hint="The number the school will actually ring">
              <input value={n.parentPhone} inputMode="tel" onChange={(e) => setN({ ...n, parentPhone: e.target.value })} placeholder="0722 000000" style={{ ...darkInput(), width: "100%" }} />
            </Field>
            <Field label="Another number" hint="For when the first cannot be reached">
              <input value={n.altPhone} inputMode="tel" onChange={(e) => setN({ ...n, altPhone: e.target.value })} style={{ ...darkInput(), width: "100%" }} />
            </Field>
            <Field label="Email">
              <input value={n.parentEmail} type="email" inputMode="email" onChange={(e) => setN({ ...n, parentEmail: e.target.value })} style={{ ...darkInput(), width: "100%" }} />
            </Field>
          </FormRow>
        </FormCard>
        ...
        <FormCard title="At the school">
          <FormRow>
            <Field label="Fee due for the term">
              <input type="number" inputMode="numeric" value={n.feeDue} onChange={(e) => setN({ ...n, feeDue: e.target.value })} style={{ ...darkInput(), width: "100%", fontFamily: FONT.mono }} />
            </Field>
            <Field label="Day or boarding">
              <select value={n.boarder ? "boarder" : "day"} onChange={(e) => setN({ ...n, boarder: e.target.value === "boarder" })} style={{ ...darkInput(), width: "100%" }}>
                <option value="day">Day scholar</option>
                <option value="boarder">Boarder</option>
              </select>
            </Field>
          ...
          <FormRow>
            <Field label="Anything the school should know" hint="Allergies, a condition needing medicine, or a note about collection">
              <textarea value={n.medical} onChange={(e) => setN({ ...n, medical: e.target.value })} style={{ ...darkInput(), width: "100%", height: 62, resize: "vertical" }} />
            </Field>
          </FormRow>
        </FormCard>

        <div>
          <button onClick={add} disabled={!n.firstName.trim() || !n.surname.trim()}
            style={{ ...primaryBtn(), fontSize: 14.5, padding: "11px 22px",
              opacity: (!n.firstName.trim() || !n.surname.trim()) ? 0.5 : 1 }}>
            Register pupil
          </button>
          <button onClick={() => setN(BLANK_STUDENT)} style={{ ...backBtnStyle(), marginLeft: 14 }}>clear the form</button>
        </div>
      </div>
