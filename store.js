...
export async function staffLogin(school, username, password) {
  ...
  const who = {
    ...
    institutionType: row.institution_type || "k12",
  };
...
//
// schoolCode is only used by the owner, reaching into a school other than
// their own — an ordinary admin resetting someone in their own school never
// needs to pass it.
export const staffResetPassword = (username, newPassword, schoolCode) =>
  rpc("staff_reset_password", { p_token: getToken(), p_username: username, p_new: newPassword, p_school_code: schoolCode || null });
...
// ---------- the owner stepping into another school ----------
// Reuses every existing admin screen rather than rebuilding pupils, marks,
// fees and staff management a second time just for the owner. The owner's
// own session is kept safe on the side and restored exactly as it was the
// moment they leave.
const OWNER_RETURN_KEY = "school_owner_return_v1";

export async function ownerEnterSchool(schoolCode) {
  const rows = await rpc("owner_enter_school", { p_token: getToken(), p_school_code: schoolCode });
  const row = Array.isArray(rows) ? rows[0] : rows;
  if (!row || !row.token) throw new Error("Could not enter that school.");

  // stash the owner's own session so leaving is a real restore, not a
  // second sign-in
  const mine = { token: getToken(), who: getWho() };
  try { localStorage.setItem(OWNER_RETURN_KEY, JSON.stringify(mine)); } catch (e) {}

  const who = {
    role: row.role, name: row.name, teacherId: row.teacher_id, username: "__owner__",
    mustChange: false, schoolCode: row.school_code, schoolName: row.school_name,
    schoolLocation: row.school_location, isOwner: false, visiting: true,
  };
  try {
    localStorage.setItem(TOKEN_KEY, row.token);
    localStorage.setItem(WHO_KEY, JSON.stringify(who));
    localStorage.setItem(SCHOOL_KEY, row.school_code);
  } catch (e) {}
  return who;
}

export const isVisitingAsOwner = () => { try { return !!localStorage.getItem(OWNER_RETURN_KEY); } catch (e) { return false; } };

export function ownerReturnHome() {
  try {
    const raw = localStorage.getItem(OWNER_RETURN_KEY);
    if (!raw) return false;
    const mine = JSON.parse(raw);
    localStorage.setItem(TOKEN_KEY, mine.token);
    localStorage.setItem(WHO_KEY, JSON.stringify(mine.who));
    localStorage.setItem(SCHOOL_KEY, mine.who?.schoolCode || "");
    localStorage.removeItem(OWNER_RETURN_KEY);
    return true;
...
export const schoolCreate = (s) => rpc("school_create", {
  ...
  p_stage: s.stage || "basic", p_institution_type: s.institutionType || "k12",
});
export const schoolSetActive = (code, active) =>
  rpc("school_set_active", { p_token: getToken(), p_code: code, p_active: active });

// ---------- college/university: a different kind of school entirely ----------
// A student here manages their own account directly — no parent, no PIN —
// the same way a teacher or admin does. Everything below only ever applies
// to a school created with institutionType "college"; every existing
// primary and secondary school keeps working exactly as it already does.
export const unitsAvailable = () => rpc("units_available", { p_token: getToken() });
export const unitRegister   = (unitId) => rpc("unit_register", { p_token: getToken(), p_unit_id: unitId });
export const unitDrop       = (unitId) => rpc("unit_drop",     { p_token: getToken(), p_unit_id: unitId });
export const myRegistrations = () => rpc("my_registrations", { p_token: getToken() });
export const myTranscript    = () => rpc("my_transcript",    { p_token: getToken() });

// staff/admin side — setting units up and grading them
export const unitUpsert = (u) => rpc("unit_upsert", {
  p_token: getToken(), p_id: u.id || null, p_code: u.code, p_name: u.name,
  p_credits: u.credits || 3, p_semester: u.semester, p_deadline: u.deadline || null,
  p_capacity: u.capacity || null, p_fee: u.fee || 0,
  p_department: u.department || null, p_is_common: !!u.isCommon,
});
export const unitsManageList = () => rpc("units_manage_list", { p_token: getToken() });
...
// A student's full admission record — beyond just a login. KCSE grade
// included, since that is what most colleges actually admit on.
export const studentProfileSet = (username, p) => rpc("student_profile_set", {
  p_token: getToken(), p_username: username,
  p_dob: p.dob || null, p_sex: p.sex || null, p_national_id: p.nationalId || null,
  p_county: p.county || null, p_program: p.program || null,
  p_admission_year: p.admissionYear ? Number(p.admissionYear) : null,
  p_kcse_grade: p.kcseGrade || null,
});
export const studentProfileGet = (username) => rpc("student_profile_get", { p_token: getToken(), p_username: username });

// One call, one transaction: the system works out the registration number
// itself — nobody types one in, so nobody can type in one that already
// belongs to someone else.
export const studentRegister = (p) => rpc("student_register", {
  p_token: getToken(), p_name: p.name,
  p_dob: p.dob || null, p_sex: p.sex || null, p_national_id: p.nationalId || null,
  p_county: p.county || null, p_department: p.department || null, p_program: p.program || null,
  p_admission_year: p.admissionYear ? Number(p.admissionYear) : null,
  p_kcse_grade: p.kcseGrade || null,
});
export const myProfile = () => rpc("my_profile", { p_token: getToken() });
export const collegeFeesList = () => rpc("college_fees_list", { p_token: getToken() });
export const unitFeePay = (registrationId, amount) => rpc("unit_fee_pay", { p_token: getToken(), p_registration_id: registrationId, p_amount: amount });
export const unitDelete = (unitId) => rpc("unit_delete", { p_token: getToken(), p_unit_id: unitId });
...
export function schoolFromUrl() {
  try {
    ...
    if (!isIp && parts.length >= minParts && !host.endsWith("vercel.app") && !host.endsWith("pages.dev")) {
...
export function schoolUrl(code) {
  try {
    ...
    if (host.endsWith("vercel.app") || host.endsWith("pages.dev") || host === "localhost" ||
